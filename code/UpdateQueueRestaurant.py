...
class Restaurant:
    ...
    def show_queue(self):
        if self.waiting_queue.is_empty():
            print("คิวปัจจุบัน: ไม่มีลูกค้าในคิว")
        else:
            print(f"คิวปัจจุบัน: {self.waiting_queue.items}")
        print(f"จำนวนลูกค้าในคิว: {self.waiting_queue.size()}")

# จากโค้ดเดิมในกรณีที่คิวว่างจะแสดง "คิวปัจจุบัน: []" ซึ่งอาจเกิดความไม่เข้าใจในการใช้งาน จึงเพิ่มเงื่อนไขในการเช็คว่าคิวว่างหรือไม่ ก่อนทำการโชว์คิวปัจจุบันมีอะไรบ้าง 
